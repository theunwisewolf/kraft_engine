#include "kraft_vulkan_index_buffer.h"
...
bool VulkanCreateIndexBuffer(VulkanContext* context, const void* data, uint64 size, VulkanBuffer* out)
{
    ...
    if (!VulkanCreateBuffer(context, size, VK_SHARING_MODE_EXCLUSIVE, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, true, out))
    {
        return false;
    }
